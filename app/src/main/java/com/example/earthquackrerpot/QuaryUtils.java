...
public final class QuaryUtils {
    ...
    private QuaryUtils(){}
        ...
        public static ArrayList<Earthquake> extractEarthquakes() {
            ...
            try
            // Mendekslarasikan Json yang didapat
            {

                JSONObject baseJsonResponse = new
                        JSONObject(SAMPLE_JSON_RESPONE);

                JSONArray earthquakeArray = baseJsonResponse
                        .getJSONArray("features");

                //Perulangan (for) Mencari yang dibutuhkan(karena yang dibutuhkan ada di Object(dengan symbol "{")
                //yang diperlukan yaitu magnitudde,location,time,url
                for (int i = 0; i < earthquakeArray.length(); i++) {
                    JSONObject currentEarthquake = earthquakeArray
                            .getJSONObject(i);
                    ...
                    //Menambahkan isi variabel 'eartquake' dengan isi 'magnitude,location,time, url'
                    Earthquake earthquake = new Earthquake(magnitude, location, time, url);
                    earthquakes.add(earthquake);
                }
            } catch (JSONException e) {
                Log.e("QuaryUtils", "Problem parsing the earthquake JSON results");
            }
